...
- `MeasureControl`: polygon centroid dot covered by the semi-transparent fill in edit mode — a div-icon marker in `measure_graph` shares the pane with the fill's SVG renderer, whose container z-index (pane z) exceeds the dot's screen-Y z-index after `sortLayers` re-sorts on zoom. Add a `zIndexOffset` (`CENTER_DOT.Z_OFFSET = 1000`) to the centroid dot so it stays above the SVG container while remaining below the area label (`CENTROID_Z_OFFSET = 2000`) and the del icon (11000) ([#230](https://github.com/Zeroto521/foliplus/pull/230), [#238](https://github.com/Zeroto521/foliplus/pull/238))
