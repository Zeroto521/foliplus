...
- `LayerControl`: styled keyboard navigation cursor — accent left bar, 8 % wash, hairline separators, entrance animation, and accent-coloured type-icon/more-button on the focused row, replacing the browser default outline ([#228](https://github.com/Zeroto521/foliplus/pull/228))
- Global keyboard focus ring: replaced the harsh solid red `outline` on `:focus-visible` with a soft double-ring — a thin accent hairline plus a semi-transparent red glow — applied uniformly across all foliplus buttons, search/heatmap/layer inputs, the layer more-menu, and the layer color picker ([#228](https://github.com/Zeroto521/foliplus/pull/228))
