...
- `LayerControl`: suppress flash on fold rebuild — remove `background-color` from `transition` on `input[type=checkbox]`, `.foliplus-layer-item`, and `.foliplus-layer-toggle-all`; also remove `border-color` from `input[type=checkbox]` transition ([#232](https://github.com/Zeroto521/foliplus/pull/232))
