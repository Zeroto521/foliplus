...
- `doc`: quickstart base themes move off CARTO tiles (`CartoDB positron` / `CartoDB dark_matter`) to bare `basemaps.cartocdn.com` URLs (`light_all` / `dark_all`) — the `CartoDB *` provider names are now token-gated upstream and raise `ValueError: Token is required for this provider` on Read the Docs builds, while the CDN tiles themselves stay open. OpenStreetMap stays the default base map, with the CARTO light/dark pairs added as switchable themes. All confirmed reachable with no credentials ([#239](https://github.com/Zeroto521/foliplus/pull/239))
