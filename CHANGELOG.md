...
- `ListCursor`: shared keyboard cursor for foliplus lists — roving tabindex + ARIA (listbox/option) in one core primitive. LayerControl rows use it (Tab enters/exits the list in one step; in-row controls are `tabindex=-1`); SearchControl results paint ARIA / active class from it
