...
- `HeatmapControl`: carry CONF on the UI state (`ctrl.conf`/`ctrl.T`) instead of a module-level free variable, making `ui.ts` unit-testable — now 100% line-covered by a dedicated `ui.test.ts` with a shared `fixture.ts` ([#309](https://github.com/Zeroto521/foliplus/pull/309))
...
- `Frontend tooling`: `test/js` now has its own strict tsconfig program (`npm run typecheck:tests`) instead of being declared exempt from `strict` — 1495 measured errors, so it still does not gate CI, but the number is now visible instead of assumed ([#305](https://github.com/Zeroto521/foliplus/pull/305))
...
- `TypeScript`: remove every `as any` and `@ts-ignore` from production sources, replacing the `map.foliplus` namespace casts with a single owning seed in `core/mapApi.ts` and typing the shortcut bookkeeping fields the manager writes ([#305](https://github.com/Zeroto521/foliplus/pull/305))
- `ExportControl`: `L.LayerEvent` resolved to `any` because `export as namespace L` only re-exports namespaces and classes, so the `layeradd` cast was a no-op — import the type instead ([#305](https://github.com/Zeroto521/foliplus/pull/305))
