...
- `MeasureControl`: polygon centroid label was painted over by the center dot after a zoom — the label and dot share the same latlng in `labelPane`, so the dot's `zIndexOffset` wins on any pixel overlap. Lift `CENTROID_ANCHOR` ([0, chipH + dotAnchor + gap] = [0, 34]) so the chip clears the dot entirely, with regression tests for the anchor invariant and the dot's `zIndexOffset` ([#230](https://github.com/Zeroto521/foliplus/pull/230))
