...
- `CSS build`: split the single 710-line `common.css` into nine semantic modules under `css/common/` — `token`, `reset`, `button`, `menu`, `input`, `hint`, `icon`, `ctrl-fold`, `panel` — merged by the build through an explicit manifest that orders them (tokens first) rather than alphabetically, so `var(--…)` references always resolve against the properties that define them. Button, menu and input states were one 320-line file named `button` although two of its three groups had nothing to do with buttons; they are now separate modules named for the surface they style. The panel stylesheet moved into the folder: it reads only shared tokens and no `ctrl-fold` rule, so it never needed to live outside the shared modules. A stylesheet on disk that the manifest does not name, or a manifest entry with no file behind it, both fail the build instead of producing a silently wrong bundle ([#266](https://github.com/Zeroto521/foliplus/pull/266))
