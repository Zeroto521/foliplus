...
- `MeasureControl`: polygon centroid dot covered by the semi-transparent fill in edit mode — the centroid dot and circle center were div-icon markers in `measure_graph`, competing with the fill's SVG renderer container z-index. After `sortLayers` re-sorts on zoom, the dot's screen-Y z-index drops below the SVG container, letting the fill paint over it. Converted both to SVG `CircleMarker` (same renderer as the fill), so DOM order within the SVG guarantees correct paint order — no z-index competition, no `zIndexOffset` needed ([#230](https://github.com/Zeroto521/foliplus/pull/230), [#238](https://github.com/Zeroto521/foliplus/pull/238))
