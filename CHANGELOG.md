...
- `FullscreenControl`: crossfade scrim on enter/exit — basemap fades down to 50% dark then back up, while controls, hints, and markers stay crisp and untouched. Mounts on the map container (not the document) so it paints in native fullscreen, scopes state per map for multi-map pages, and respects `prefers-reduced-motion`. z-index and fade tokens live in the stylesheet, JS owns only mount/toggle/teardown
