...
- `FullscreenControl`: standing rotate hint when entering fullscreen in portrait on mobile — cleared on exit or on a rotation that lands in landscape, stacked beside the enter toast instead of replacing it ([#326](https://github.com/Zeroto521/foliplus/pull/326))
...
- `CSS build`: split the single 710-line `common.css` into nine semantic modules under `css/common/` ([#266](https://github.com/Zeroto521/foliplus/pull/266), [#323](https://github.com/Zeroto521/foliplus/pull/323))
...
- `build`: per-bundle size band in the CI bundle-size check (`--bundleThreshold=file:pct`) — the global 10% band is a percentage, so a small bundle like FullscreenControl's 1 KB has barely any headroom ([#326](https://github.com/Zeroto521/foliplus/pull/326))
