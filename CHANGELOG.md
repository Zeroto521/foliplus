...
- `LayerControl`/`MeasureControl`: `createLayers` now takes an ordered `panes: {name, isLabel?}[]` list instead of a single label-pane name; MeasureControl routes geometry, nodes, and labels to three panes whose z-index (graph < node < label) guarantees paint order, fixing preview labels covered by nodes ([#280](https://github.com/Zeroto521/foliplus/pull/280))
