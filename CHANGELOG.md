...
- `Build pipeline`: the bundle size check no longer reports unrelated bundles as changed — sizes are measured with the esbuild version banner stripped, the base branch and the PR build share one `node_modules` so the toolchain cannot drift between them, and a recorded toolchain mismatch is warned about instead of being silently measured ([#257](https://github.com/Zeroto521/foliplus/pull/257))
