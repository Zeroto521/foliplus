...
- `Build pipeline`: enable esbuild Tree Shaking via auto-scanned import detection — component imports are scanned at build time to generate targeted named imports, dropping unused exports from shared modules. `foliplus-common.min.js` reduced ~35% (32KB→21KB), total JS bundle ~5.3% (214KB→202KB). ([#162](https://github.com/Zeroto521/foliplus/pull/162))
- `Build pipeline`: eliminate `.build/` source mirror — SVG/HTML transforms now run at esbuild bundle time via `onLoad` plugin; per-entry `treeShaking` (off for shared entry, on for components). Build time reduced ~15%. ([#162](https://github.com/Zeroto521/foliplus/pull/162))
- `Build tooling`: extract shared CLI argument parser (`args.mjs`), standalone registry generator (`scan-registry.mjs`), unified `--help`/`--root`/`--check`/`--dev` interface with short-flag support. ([#162](https://github.com/Zeroto521/foliplus/pull/162))
- `Build testing`: add 77 new vitest tests covering args parsing, SVG/HTML transforms, namespace plugin export collection, import scanning, and artifact verification (716→785 total). ([#162](https://github.com/Zeroto521/foliplus/pull/162))
