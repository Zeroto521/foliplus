...
- `MeasureControl`: polygon centroid area label appeared washed out through the polygon fill after a zoom-out, and a prior fix lifting it with `zIndexOffset` (11000) made it cover other labels like the segment-distance chip. Keep both the centroid dot and label at z = Y (no `zIndexOffset`), same level as all other measure labels — the centroid must never hide a segment label. The label is added after the dot so at equal z-index DOM order puts it above the dot when they share the centroid latlng; the ✕ delete icon keeps its own high offset to stay above both. Regression tests assert the centroid markers carry no `zIndexOffset` ([#230](https://github.com/Zeroto521/foliplus/pull/230))
