...
- `sanitize`: shared allowlist SVG gate (`common/sanitize.ts`) for the three sinks that accept externally-supplied markup — `iconSvg` on `LayerAPI.registerLayer`/`createLayers`, registered hint icons, and reverse-geocoded marker addresses ([#286](https://github.com/Zeroto521/foliplus/pull/286))
...
- `sanitize`: `foreignObject` was never dropped — the blacklist stored source casing while the check compares `tagName.toLowerCase()`, which `image/svg+xml` keeps case-preserved, so the rule was dead code ([#286](https://github.com/Zeroto521/foliplus/pull/286))
