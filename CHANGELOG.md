...
- `FullscreenControl`: fullscreen transition — a flat black scrim (`rgba(0,0,0,var(--alpha-50))`) that fades in on enter and stays dimmed while fullscreen is active, fading out symmetrically on exit (180ms ease-out); state-bound to fullscreen rather than a momentary flash, matching the pattern ExportControl uses during crop mode. Scrim lives at z-index 799 below controls; survives bundler minification; re-drives the dim from the Fullscreen API in rejection paths and in `fullscreenchange` so a denied exit or Esc race never leaves the basemap darkened ([#217](https://github.com/Zeroto521/foliplus/pull/217))
