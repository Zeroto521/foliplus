...
- `HeatmapControl`: keep the single-layer auto-select as a one-shot first-load behavior so a user's manual clear is not overridden by later rebuilds (zoomend, layeradd/layerremove)
