...
- `FullscreenControl`: fullscreen transition — a flat black scrim (`rgba(0,0,0,var(--alpha-50))`) that flashes on enter and exit (180ms fade-in, auto 180ms fade-out, then detaches); one-shot transition signal rather than a persistent overlay, matching foliplus' event-driven animation DNA. Scrim lives at z-index 799 below controls; survives bundler minification; skipped entirely on denied exits so rejection paths leave no visible side effect ([#217](https://github.com/Zeroto521/foliplus/pull/217))
