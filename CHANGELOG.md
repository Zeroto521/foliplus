...
- `MeasureControl`: edit mode — a pencil toolbar button enters an edit state where clicking a measurement reveals its × delete handles and dragging its nodes repositions it (labels update live, changes persist on release). Circle: drag the center to translate the whole shape, drag the radius node to resize. Area: drag a node to reshape, drag the centroid to translate the whole polygon. Marker: drag to move and re-resolve the address on drop. Edit and drawing modes are mutually exclusive; Escape exits edit mode; only one measurement shows × at a time ([#196](https://github.com/Zeroto521/foliplus/pull/196))
