...
- Package: ship a PEP 561 `py.typed` marker and re-export `LocaleConfig` plus the public type aliases (`Position`, `Zoom`, `PositiveInt`, `Fraction`) from the top level, so downstream type checkers (mypy, pyright, ...) analyze foliplus instead of skipping it
