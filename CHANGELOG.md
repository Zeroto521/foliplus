...
- `doc`: quickstart base themes move off CARTO tiles (`CartoDB positron` / `CartoDB dark_matter`) to `Esri World Gray Canvas` / `World_Dark_Gray_Base` — the CARTO basemap is behind a paid API token, which raised `ValueError: Token is required for this provider` on Read the Docs builds. Both themes confirmed reachable with no credentials ([#239](https://github.com/Zeroto521/foliplus/pull/239))
