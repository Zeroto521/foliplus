...
- `ExportControl`/`HeatmapControl`/`LayerControl`/`MeasureControl`: carry CONF on the UI state (control or manager instance) instead of module-level free variables, making each control's UI code unit-testable with per-instance configuration; their `ui.ts`/`ui/*` modules now ship dedicated unit suites and rejoin the coverage report ([#309](https://github.com/Zeroto521/foliplus/pull/309), [#311](https://github.com/Zeroto521/foliplus/pull/311))
