...
- `MeasureControl`: extract measurement data lifecycle (array, id counter, localStorage persistence, count-event emission) into a `MeasureStore` class; modes/export go through the typed `store.add/remove/update/all` API instead of poking `manager.measurements` + `saveMeasurements()` directly, mirroring `LayerControl`'s single-store convention. A thin compat shell keeps legacy/test call sites working; persisted entries missing an `id` are stabilized on restore ([#234](https://github.com/Zeroto521/foliplus/pull/234))
