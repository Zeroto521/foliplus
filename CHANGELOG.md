...
- LayerControl row interaction cursor unified — mouse hover, Tab focus and the arrow-key cursor now share one look (white row, red top/bottom glow, drag handle, type icon black, more button red), matching the arrow-key reference. The default dark focus outline is suppressed; the left accent bar stays a checked-status indicator only. The fold (toggle-all) row joins the recipe and its fold icon wakes red consistently. Clicking outside the panel drops the cursor. The old common.css cursor block and `--cursor-bar-width`/`--cursor-glow` tokens were removed — the recipe lives in `LayerControl.css`. LayerControl: Enter/Space on the group fold chevron folds the group instead of toggling its select-all checkbox ([#229](https://github.com/Zeroto521/foliplus/pull/229))
