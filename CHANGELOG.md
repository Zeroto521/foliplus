...
- `HeatmapControl`: auto-select single point layer on panel expand, skipping the manual selection step ([#133](https://github.com/Zeroto521/foliplus/pull/133), [#231](https://github.com/Zeroto521/foliplus/pull/231))
...
- `ModeManager`: prevent conflicting component actions (e.g. measurement during export) via mode tracking and mutual-exclusion blocking ([#150](https://github.com/Zeroto521/foliplus/pull/150), [#159](https://github.com/Zeroto521/foliplus/pull/159), [#224](https://github.com/Zeroto521/foliplus/pull/224))
- `LayerControl`: keyboard shortcuts for layer panel — `ArrowUp`/`ArrowDown`/`ArrowLeft`/`ArrowRight`/`Space`/`Enter`/`Escape` and `Ctrl+ArrowUp`/`Ctrl+ArrowDown` for reorder. Added `moveLayerUp(id)` and `moveLayerDown(id)` to `LayerAPI` ([#156](https://github.com/Zeroto521/foliplus/pull/156), [#215](https://github.com/Zeroto521/foliplus/pull/215))
...
- `LayerControl`: persist layer configuration to localStorage across page reloads — five dimensions — order, fold, visibility, names, annotation — with per-map-container scoping, centralized in a `LayerPersistence` class; stale ids from removed layers are pruned on load ([#223](https://github.com/Zeroto521/foliplus/pull/223), [#225](https://github.com/Zeroto521/foliplus/pull/225))
...
- `LayerControl`: per-layer style panel from a layer's ⋮ menu — toggle text labels on features and choose the label field and number format ([#236](https://github.com/Zeroto521/foliplus/pull/236))
...
- LayerControl: mouse hover, Tab focus and the arrow-key cursor now share one row highlight (white row, red top/bottom glow, drag handle, type icon black, more button red). The dark focus outline is suppressed; the left accent bar stays a checked indicator; the fold row and its fold icon wake red consistently ([#229](https://github.com/Zeroto521/foliplus/pull/229))
...
- `LayerControl`: a hide or reorder made inside the 100ms persistence debounce window was lost on reload — `destroy()` cancelled the timers before the pending write could fire. The writes are now flushed before teardown, and `LayerPersistence` collapses to one `load()` / one `write()` funnel per side plus a single `flushAll()` at teardown, so adding a persisted dimension cannot be missed on any of them ([#265](https://github.com/Zeroto521/foliplus/pull/265))
- `LayerControl`: hiding a runtime-registered layer and reloading dropped the user's checked state on re-registration — the prune kept ids only while a row was still rendered, but a row implies a registry entry, so the clause was dead and the sweep pruned everything. Liveness now means "in the registry or still queued in `pendingRegistrations`", which `attachUI` drains before the sweep ([#265](https://github.com/Zeroto521/foliplus/pull/265))
- `LayerControl`: the round trip was half-implemented — hiding a layer persisted, but checking on a layer the author declared `show=False` did not, so `Commuting Routes` and `Facility Points` in the quickstart came back unchecked on every reload. The persisted set is now the absolute set of ids currently off the map rather than the ids the user toggled to hide, and the apply sweep gained the inverse path that puts an un-hid layer back on the map. An absent visibility key is distinguished from an empty one so the author's `show=False` defaults survive a first load the user never touched ([#265](https://github.com/Zeroto521/foliplus/pull/265))
