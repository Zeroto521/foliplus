...
- `MeasureControl`: circle preview radius line covered the radius label and the circle center — a layer whose renderer was never pinned joined a stale fallback renderer, so a second attach hit an already-held `<path>` and broke the whole SVG renderer; `PaneManager.ensureVector` now pins both `renderer` and `pane` on the layer, preview labels route to the label pane, and the attachment order is declared once as `LAYER_STACK` and asserted by tests ([#252](https://github.com/Zeroto521/foliplus/pull/252))
