...
- `LayerControl`/`HeatmapControl`: attach-time init is signal-driven — `BaseControl` emits `CONTROL_ATTACHED`; LayerControl re-runs its init (marks the panel `data-ready`) and Heatmap re-scans for point layers, replacing the fixed 300ms init timer and the 8 × 300ms retry loop; browser tests share one `panel_ready` helper ([#283](https://github.com/Zeroto521/foliplus/pull/283), [#285](https://github.com/Zeroto521/foliplus/pull/285))
