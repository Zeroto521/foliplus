...
- `LayerControl`: extract layer core into `core/layer/`; mount `LayerAPI` on `map.foliplus` with a thin facade, always available via `ensureLayerAPI` ([#138](https://github.com/Zeroto521/foliplus/pull/138), [#139](https://github.com/Zeroto521/foliplus/pull/139), [#140](https://github.com/Zeroto521/foliplus/pull/140))
  - **Why**: `LayerAPI` used to require LayerControl; now the core lives in DOM-free `core/layer/` and `ensureLayerAPI` guarantees a usable API even without the layer panel.
