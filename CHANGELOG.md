...
- `MeasureControl`: label collision detection — heavily-overlapping chips are hidden instead of nudged so labels stay on their anchor; priority-ordered hiding keeps the centroid, radius, and the distance total visible first. The geometry now lives in the shared `core/labelCollision` module (with caller-controlled overlap threshold and viewport culling), so LayerControl's annotation labels can plan collisions the same way ([#221](https://github.com/Zeroto521/foliplus/pull/221), [#331](https://github.com/Zeroto521/foliplus/pull/331))
