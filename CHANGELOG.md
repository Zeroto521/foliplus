...
- `MeasureControl`: edit mode — enter edit mode to click a measurement and reveal its delete handles, then drag nodes to reposition (labels update live). Circle: drag center to translate the whole shape, drag the radius node to resize. Marker: drag to move and re-resolve the address on drop. Edit and drawing modes are mutually exclusive ([#196](https://github.com/Zeroto521/foliplus/pull/196))
