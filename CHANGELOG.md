...
- `MeasureControl`: export measurements to GeoJSON or CSV — selectable via the `export_format` parameter; the CSV carries a WKT column plus a UTF-8 BOM so Excel detects the encoding ([#168](https://github.com/Zeroto521/foliplus/pull/168), [#253](https://github.com/Zeroto521/foliplus/pull/253), [#250](https://github.com/Zeroto521/foliplus/pull/250))
...
- `MeasureControl`: export showed no feedback — a successful export was silent and a serialization failure surfaced nothing but a `console.warn`; now reports the filename and measurement count on success, and a retry hint when the file was not saved ([#250](https://github.com/Zeroto521/foliplus/pull/250))
