...
- `MeasureControl`: circle preview radius line covered the radius label and the circle center — a layer whose renderer was never pinned joined a stale fallback renderer, so a second attach hit an already-held `<path>` and broke the whole SVG renderer. `PaneManager.ensureVector` now pins both `renderer` and `pane` on the layer; the circle center moves to a dedicated `nodePane` above `graphPane`; and `enforceOrder` pins each layer's child panes one step above its graph pane, so the pane z set at creation time is not overwritten back to 600 ([#252](https://github.com/Zeroto521/foliplus/pull/252))
