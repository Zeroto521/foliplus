...
- `LayerControl`: per-layer annotation style panel — open a floating panel from a layer's ⋮ menu to show text labels on features, picking the label field and number format (auto/integer/thousands/percent). Panel is anchored to its triggering row with viewport flipping, and the configuration is persisted per-map to localStorage ([#236](https://github.com/Zeroto521/foliplus/pull/236))
