...
- `LayerControl`/`SearchControl`/Global: beautify keyboard navigation — LayerControl cursor and SearchControl result-item cursor (1 px accent bar + 8 % wash + hairlines, hover/keyboard cursor visually separated), unified `--focus-ring` soft double-ring on all foliplus `:focus-visible` (buttons, inputs, more-menu, color picker, scheme bar) replacing the harsh red outline, and removed misleading red more-button tint on `.active` ([#228](https://github.com/Zeroto521/foliplus/pull/228))
