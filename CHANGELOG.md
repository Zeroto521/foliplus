...
- LayerControl: mouse hover, Tab focus and the arrow-key cursor now share one row highlight (clear surface, red top/bottom glow, drag handle, type icon black, more button red). The dark focus outline is suppressed; the left accent bar stays a checked indicator; the fold row and its fold icon wake red consistently. The recipe CSS keys only on `:hover` + the JS class `.foliplus-layer-focused` — `:focus-visible` is sampled once in the `focusin` delegate and mapped onto that class, so Escape is a single class removal ([#229](https://github.com/Zeroto521/foliplus/pull/229), [#269](https://github.com/Zeroto521/foliplus/pull/269), [#276](https://github.com/Zeroto521/foliplus/pull/276))
