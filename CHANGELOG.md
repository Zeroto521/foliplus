...
- Naming: singularize the split module filenames — `css/LayerControl/{row,attr,control}.css` and `js/LayerControl/ui/attr.ts` (were `rows`/`attrs`/`controls`); no behaviour change
