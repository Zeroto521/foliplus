...
- `LayerControl`/`HeatmapControl`: one shared panel shell — `common/panel.ts` builds the fold button, header, content area and outside-click collapse for both, replacing `LayerControl`'s hand-rolled markup (its `template.ts` is gone); the shell now also derives a panel id, labels the close button and returns its unbind so `BaseControl` tears it down on remove; a press inside either panel no longer collapses the other ([#303](https://github.com/Zeroto521/foliplus/pull/303))
