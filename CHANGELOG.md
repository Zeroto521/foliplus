...
- `CSS build`: split the single 710-line `common.css` into six semantic modules under `css/common/` — `tokens`, `reset`, `button`, `hint`, `icons`, `ctrl-fold` — merged by the build through an explicit manifest that orders them (tokens first, panel last) rather than alphabetically, so `var(--…)` references always resolve against the properties that define them. A stylesheet on disk that the manifest does not name, or a manifest entry with no file behind it, both fail the build instead of producing a silently wrong bundle ([#266](https://github.com/Zeroto521/foliplus/pull/266))
