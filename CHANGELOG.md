...
- `MeasureControl`: `show_live_coords` parameter (default `true`) — a small chip trails the cursor while a drawing mode or edit mode is armed, hanging south of the pointer like the area label on the centroid dot, so the coordinate about to be placed is visible; it reports the map's own coordinates with no CRS conversion and disappears once the measurement is finalized ([#246](https://github.com/Zeroto521/foliplus/pull/246))
