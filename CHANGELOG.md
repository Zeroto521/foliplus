...
- `doc`: quickstart base themes move off CARTO tiles (`CartoDB positron` / `CartoDB dark_matter`) to Esri World Gray / Dark Gray Canvas — the CARTO basemap is behind a paid API token, which raised `ValueError: Token is required for this provider` on Read the Docs builds. Zero-config, no key ([#239](https://github.com/Zeroto521/foliplus/pull/239))
