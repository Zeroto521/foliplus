...
- `build`: the bundle size report is published to the PR comment before the threshold gate is evaluated, so a breach no longer aborts the job and leaves the comment on the last green verdict — `--enforce` re-adds the exit code for the gate ([#330](https://github.com/Zeroto521/foliplus/pull/330))
