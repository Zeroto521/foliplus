...
- `Build artifacts`: prefix all built assets with `foliplus-` (`foliplus-{Name}.min.js` / `foliplus-{Name}.min.css`) and inject a version banner into every artifact ([#147](https://github.com/Zeroto521/foliplus/pull/147))
- `Shared bundle`: introduce `foliplus-common.min.js` containing ALL shared code exactly once — `core/layer`, `common/*` helpers, `BaseControl`, hint module, and the geocode singleton. Component bundles externalize `#core/*` / `#common/*` to the `foliplus.core.*` / `foliplus.common.*` global namespace instead of inlining copies, cutting total JS from ~193KB to ~110KB (−43%). ([#147](https://github.com/Zeroto521/foliplus/pull/147))
- `hint`: made per-map — global `window.foliplus.showHint/hideHint` removed; each map gets its own `HintManager` via `ensureHint(map)`, exposed as `map.foliplus.showHint/hideHint`. `HINT_DURATION` consolidated into `core/hint.ts`. ([#147](https://github.com/Zeroto521/foliplus/pull/147))
- `geocode`: add forward geocoding (`foliplus.geocode`) alongside reverse; both share a global FIFO cache with a 24h TTL and a unified 1 req/s throttle. ([#147](https://github.com/Zeroto521/foliplus/pull/147))
- `Cache`: new generic `Cache<K, V>` container (FIFO eviction + optional TTL) for bounded caching ([#147](https://github.com/Zeroto521/foliplus/pull/147))
