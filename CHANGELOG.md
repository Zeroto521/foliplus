...
- `LayerControl`/`HeatmapControl`: both side panels now build their shell from one shared `createPanelControl` helper, so the panel header is a labelled `role="dialog"` on every panel and clicking outside the map collapses them the same way ([#303](https://github.com/Zeroto521/foliplus/pull/303))
