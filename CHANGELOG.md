...
- `MeasureControl`: edit mode — click a measurement to reveal its × handles and drag nodes to reposition it (labels update live, changes persist on release). Circle: drag the center to translate, the radius node to resize. Area: drag a node to reshape, the centroid to translate. Marker: drag to move and re-resolve the address. Modes are mutually exclusive; Escape exits; one × set at a time ([#196](https://github.com/Zeroto521/foliplus/pull/196))
