...
- `Frontend tooling`: add eslint 9 flat config with `padding-line-between-statements` scoped to function/class definitions (Python `E302`/`E305` equivalent, auto-fixed by a pre-commit hook) plus a quality-rules pass mirroring prettier's own repo (`eqeqeq:smart`, `curly:multi-line` so braceless one-liners stay legal, `no-implicit-coercion` with `!!` allowed, `no-else-return`, `object-shorthand`, `prefer-const`, `no-console` in runtime, `no-irregular-whitespace`) and a type-aware Promise-discipline pass (`no-floating-promises`, `require-await`, `no-misused-promises`), enforced via `npm run lint`; the pre-commit hook runs a separate dependency-free config (`no-var`, `no-debugger`) because pre-commit.ci's `NODE_PATH`-delivered deps cannot be resolved by a bare ESM import. Pins `typescript` to 6.0.3 for `typescript-eslint` compatibility ([#259](https://github.com/Zeroto521/foliplus/pull/259))
