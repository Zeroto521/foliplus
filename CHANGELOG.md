...
- `FullscreenControl`: fullscreen transition — a soft radial vignette scrim that fades the basemap in on enter and out on exit while controls stay visible, plus four L-shaped corner brackets (brand red, viewfinder metaphor) that slide in/out in sync to give the transition a spatial cue; scrim uses pure black (`rgba(0,0,0,…)`) to match the common overlay convention and avoid tinting the basemap with a color foliplus doesn't use; auto-clears after the CSS transition, survives bundler minification (`.26s` ↔ `260ms`), respects `prefers-reduced-motion`, and re-drives the dim from the Fullscreen API in rejection paths so a denied exit never leaves the basemap darkened ([#217](https://github.com/Zeroto521/foliplus/pull/217))
