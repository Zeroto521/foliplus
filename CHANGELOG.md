...
- `Frontend tooling`: add eslint 9 flat config split into a non-type-aware pass (`padding-line-between-statements` scoped to function/class definitions, `eqeqeq:smart`, `curly:multi-line`, `no-implicit-coercion` with `!!` allowed, `no-else-return`, `object-shorthand`, `prefer-const`, `no-console` in runtime, 14 zero-cost guards) and a type-aware Promise-discipline pass (`no-floating-promises`, `require-await`, `no-misused-promises`) run after `tsc --noEmit`; ESLint moves out of pre-commit.ci into GitHub Actions because its config imports packages pre-commit.ci cannot resolve by package name, and CI splits into `format` / `lint` / `typecheck` jobs. Pins `typescript` to 6.0.3 ([#259](https://github.com/Zeroto521/foliplus/pull/259))
