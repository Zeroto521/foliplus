...
- `SearchControl`: scope the search-history localStorage key to the map container (`foliplus_search_<map id>`) so multiple maps on one page do not share history, with a one-time read of the old unscoped key so existing history is carried over ([#299](https://github.com/Zeroto521/foliplus/pull/299))
...
- `LayerControl`: split `LayerUI` into `ui/*` modules (`list`/`state`/`keyboard`/`focus`/…) with a single `ui/index.js` export; public API unchanged ([#291](https://github.com/Zeroto521/foliplus/pull/291), [#296](https://github.com/Zeroto521/foliplus/pull/296))
