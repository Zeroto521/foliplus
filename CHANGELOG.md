...
- `MeasureControl`/`ExportControl`: while a measure mode is active — or the export crop box is being selected — interaction on all other map layers is temporarily suspended, so clicks fall through to the map and place measurement nodes instead of firing feature handlers (popups, delete icons, toggles). Implemented as a mode-driven interaction lock in `ModeManager`, built on shared `core/layer` `setInteractive`/`suspendMapInteractions` primitives ([#203](https://github.com/Zeroto521/foliplus/pull/203))
