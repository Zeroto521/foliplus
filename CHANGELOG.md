...
- LayerControl row interaction cursor unified — the three ways a layer row becomes the interaction target (mouse hover, Tab focus, arrow-key cursor) now share one recipe and read identically, matching the arrow-key reference style: the whole row turns white, a red glow hugs the top and bottom edges, the drag handle appears, the type icon goes black and the more button goes red. The bottom glow is occluded for middle rows (the row below covers it), so only the top edge reads on a non-last row. The default dark browser focus outline is suppressed (`outline: none`) on the focused rows, so only the custom white + glow marks the row. The left accent bar is a persistent _checked_ indicator — it stays red only for `.active` (checked) rows and folded groups, never for a transient hover/keyboard cursor. The fold (toggle-all) row joins the same recipe, and its fold icon now wakes up (red) consistently across hover, Tab and arrow-key cursor. The previous shared common.css cursor block and its `--cursor-bar-width`/`--cursor-glow` tokens were removed — the recipe lives in `LayerControl.css`, and data rows are now opaque so the glow occlusion is reliable. LayerControl: Enter/Space on the group fold chevron folds the group instead of toggling its select-all checkbox, and keyboard navigation enumerates rows by element (Tab / arrow keys cover the same rows). Single-consumer styling stays component-local — the LayerControl checkbox and the HeatmapControl label toggle (10px pill rounding) are back in their own stylesheets instead of `common.css` ([#229](https://github.com/Zeroto521/foliplus/pull/229))
