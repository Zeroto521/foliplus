...
- `CSS build`: split the single 710-line `common.css` into six semantic modules under `css/common/` — `tokens`, `reset`, `button`, `hint`, `icons`, `ctrl-fold` — merged by the build through an explicit manifest that orders them (tokens first, panel last) rather than alphabetically, so `var(--…)` references always resolve against the properties that define them. `common.css` is kept as a generated alias for tooling that still reads it
