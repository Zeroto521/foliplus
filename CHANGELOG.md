...
- `MeasureControl`: polygon centroid area label appeared washed out through the polygon fill after a zoom-out — after a Leaflet zoom animation the label's `marker-icon` landed at z = Y (equal to the fill's parent SVG z), so the fill painted over the label and `backdrop-filter` made it look faded. Give the centroid label marker the same `zIndexOffset` (11000) as the center dot so it paints in its own stacking context above the fill at any zoom; the label keeps the default anchor so it sits over the dot (equal z-index, DOM order), and the ✕ delete icon remains on top and clickable. Regression tests assert the label's `zIndexOffset` ([#230](https://github.com/Zeroto521/foliplus/pull/230))
