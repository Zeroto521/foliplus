...
- `Python`: validate constructor arguments from their own type annotations — a shared `@validate` decorator enforces the `Literal` choices and numeric bounds declared on each parameter, so an unknown `position` or an out-of-range `quality`/`scale`/`timeout`/`max_pixels`/`opacity`/`border_weight`/`label_size` raises `ValueError`, and numpy scalars are accepted where a builtin `int` used to be required ([#284](https://github.com/Zeroto521/foliplus/pull/284))
