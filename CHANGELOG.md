...
- `LayerControl`: feature count column in the layer panel — shows the number of geometric features per layer row. Base maps are excluded; canvas layers (e.g. `HeatmapControl`) and third-party layers can supply a `featureCountProvider` callback so the component counts its own data instead of falling back to a geometry walk. The row count compacts at 10^3 features (en 12K, zh 1.2万) so a 12,000-feature layer no longer reads as the clipped 999 — the attribute panel still shows the exact grouped count ([#172](https://github.com/Zeroto521/foliplus/pull/172), [#190](https://github.com/Zeroto521/foliplus/pull/190), [#324](https://github.com/Zeroto521/foliplus/pull/325))
