...
import { installLeafletGlobals } from "./ui/fixture.js";

// ===========================================================================
// LayerAPI.setVisible — the programmatic write side of the visibility contract.
//
...
// does not own. These tests pin the new entry point, and the bottom half pins
// the checkbox path it now shares.
//
...
describe("LayerManager.setVisible", () => {
  ...
  it("re-shows a hidden layer and clears paneSet for the z-order re-push", () => {
    ...
    expect(manager.setVisible("overlay1", true)).toBe(true);
  ...
  it("treats a base layer like an overlay, map membership included", () => {
    ...
    expect(fresh.setVisible("base1", true)).toBe(true);
    expect(map.hasLayer(layer)).toBe(true);
    fresh.ui = null;
    fresh.destroy();
  });
  ...
  it("fires the callback on every call, not only on change", () => {
    ...
    expect(manager.setVisible("repeat", false)).toBe(true);
    expect(manager.setVisible("repeat", false)).toBe(true);
