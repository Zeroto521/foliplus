...
describe("attachPolygonUI", () => {
  ...
  it("routes the centroid dot to the graph pane and the label to the label pane", () => {
    ...
    // pane (no isLabel flag) with a zIndexOffset to stay above the fill's
    // SVG renderer container (z = pane z). Without it the dot's z = screen Y
    // falls below the SVG and the fill paints over the dot. The label is
    // isLabel so it lands in the label pane, which always paints above the
    // graph pane. The label also carries a modest zIndexOffset so sortLayers
    // (which re-sorts by Y on zoom) doesn't push a lower-Y segment label
    ...
    // Dot: has zIndexOffset (above SVG container), routed to graph pane.
    expect(dotOpts.zIndexOffset).toBe(CONST.CENTER_DOT.Z_OFFSET);
