...
class TestLayerControlPython:
    ...
    def test_render_collects_layers_attached_to_control(self):
        ...
        object, and Map.get_root() returns the enclosing Figure ‚Ä?a different
...
class TestLayerControlRendering:
    ...
    def test_fold_icon_single_svg_css_rotation(self):
        """Fold uses a single SVG icon rotated by CSS ‚Ä?no separate UNFOLD SVG."""
    ...
    def test_locale_zh(self):
        ...
        assert_locale(html, "ÂõæÂ±Ç", "LayerControl.panel_title")
    ...
    def test_annotation_locale_keys(self):
        ...
        panel alike ‚Ä?``test_locale.py`` covers that cross-table key set.
        """
    ...
    def test_css_interaction_effects(self, base_map: folium.Map):
        ...
        :focus-visible is deliberately NOT a CSS trigger ‚Ä?Tab focus is mapped
        ...
        # ‚îÄ‚îÄ The unified Row-cursor recipe, read from the source CSS ‚îÄ‚îÄ
        ...
        # opening brace of the :is() rule (NOT the parent's ‚Ä?the nearest
        ...
        # with `.foliplus-layer-item,` ‚Ä?anchor there so css.find() does not
        ...
        # :focus-visible is never a recipe trigger ‚Ä?Esc cancel is one class off.
        ...
        # interaction recipe must NOT force it ‚Ä?hover/keyboard/Tab show the
        ...
        # only at rest ‚Ä?the cursor paints white on top.
        ...
        # ‚îÄ‚îÄ Type icon ‚îÄ‚îÄ
        assert "foliplus-type-icon-col svg" in html
        assert "transition: transform" in html
        # A checked row carries .active ‚Ä?a persistent checkbox state, not a
        # cursor ‚Ä?and the cursor-only type-icon tint comes from the recipe.
        ...
        # ‚îÄ‚îÄ More (‚ã? menu icon: identity wakes gray ‚Ü?black on hover/keyboard
        # focus, matching the LayerControl type-icon language (NOT accent) ‚Ä?the
        # main text goes black ‚Ü?red via the unified dropdown hover (css/common/menu.css),
        # so "icon black + text red" reads like every other dropdown. ‚îÄ‚îÄ
        more_icon = [
            html[i : html.index("}", i)]
            for i in range(len(html))
        ...
        # Count column: stays muted in every state ‚Ä?no hover/active brightening.
        count_hover = [
            html[i : html.index("}", i)]
            for i in range(len(html))
    ...
    def test_folded_state_no_accent_text(self):
        ...
        css = read_css("foliplus/css/LayerControl/index.css")
        # left border and fold-btn turn accent when folded ‚Ä?both expected
    ...
    def test_toggle_all_grid_uses_named_slots_for_three_items(self):
        ...
        # No template-areas line references 'divider' as a named area ‚Ä?
    ...
    def test_toggle_all_divider_uses_explicit_column_range(self):
        ...
        grid-area or grid-column: span ‚Ä?so it never overflows to a new row
    ...
    def test_toggle_all_divider_column_range_bounded_by_track(self):
        ...
        assert idx != -1
        track = css[idx : css.index(";", idx)]
        # Count the track's space tokens ‚Ä?each column is one term separated
    ...
    def test_toggle_all_align_self_center_removed(self):
        """Divider no longer declares a redundant align-self: center ‚Ä?the
    ...
    def test_open_overlay_lifts_owner_row_above_sibling_wake(self):
        ...
        # The ‚ã?dropdown anchors flush to its row (the shared shell adds a 2px
    ...
    def test_slider_component_css(self):
        ...
        drift ‚Ä?they had already grown different rail heights, different handle
        ...
        # Both engines are styled ‚Ä?and the two rules must stay SEPARATE: a
    ...
    def test_rows_declare_no_slider_geometry(self):
        ...
        raw ‚Ä?`read_css` expands `@import`, so through it every row "contains"
        token.css.
        """
    ...
    # ‚îÄ‚îÄ Drag-over animation tests ‚îÄ‚îÄ
    ...
    # ‚îÄ‚îÄ Indeterminate checkbox (partial selection) styles ‚îÄ‚îÄ
    ...
    def test_no_rebuild_flash_transitions_on_rebuilt_elements(self):
        ...
        MUST NOT transition that property ‚Ä?otherwise it animates from its
    ...
    def _assert_no_bg_transition(css, selector_fragment, must_not, may):
        ...
        # transition ‚Ä?that is the one this assertion targets.
        pos = 0
        trans_decl = None
        while True:
        ...
        # Transition value is "prop timing, prop timing, ..." ‚Ä?split on
        ...
        for prop in must_not:
            assert prop not in transitioned_props, (
                f"{selector_fragment} must not transition {prop} ‚Ä?"
                "element is destroyed+recreated on list rebuild (fold click)"
            )
        for prop in may:
            assert prop in transitioned_props, (
                f"{selector_fragment} should still transition {prop} ‚Ä?"
...
class TestLayerControlBrowser:
    ...
    def _sample_token(page, name: str) -> str:
        """Computed color of a design token ‚Ä?never hardcode a hex/rgb."""
    ...
    def _sample_neutral0(page):
        """Computed color of `var(--neutral-0)` ‚Ä?never hardcode a hex/rgb."""
    ...
    def test_cross_group_drag_shows_hint(self, browser, tmp_path):
        ...
        with use_page(self._make_page, browser, tmp_path, overlay, base) as (
            ...
            assert ("same group" in hint_text.lower()) or ("ÂêåÂàÜÁª? in hint_text)
    ...
    def test_remove_readd_leaves_no_listener_residue(self, browser, tmp_path):
        """N=3 remove‚Üíadd cycles must not grow map._events listener sum.
        ...
        rounds[1] and rounds[2] follow remove‚Üíadd. Any drift ‚Ä?whether on
        round[0] or in a later round ‚Ä?fails the assertion.

        The gate is what caught the `dismissFocus` ‚Ü?`ensureModes` first-call
    ...
    def test_geojson_group_pins_children_to_declared_pane(self, browser, tmp_path):
        ...
        Leaflet ignores a group's pane for its children ‚Ä?each child joins the
    ...
    def test_icon_svg_payload_never_reaches_dom(self, browser, tmp_path):
        ...
        model ‚Ä?callers of ``LayerAPI.registerLayer`` / ``createLayers`` hand it
        ...
        with use_page(self._make_page, browser, tmp_path, slug="xss_icon") as (
            ...
            # CSS rule. This is a real-browser serialisation property ‚Ä?jsdom
            # writes both shapes identically ‚Ä?so the assertion only exists here.
    ...
    def test_rename_input_fills_row_height(self, browser, tmp_path):
        ...
        padding rule outranking a bare `.foliplus-layer-renaming` rule ‚Ä?if that
        ...
        with use_page(
            ...
            # Row's vertical padding is dropped while renaming‚Ä?
            assert m["padTop"] == "0px", f"row padTop={m['padTop']}"
            assert m["padBottom"] == "0px", f"row padBottom={m['padBottom']}"
            # ‚Ä¶so the input fills the whole row, not a 19.6px line-box.
    ...
    def test_annotation_click_through(self, browser, tmp_path):
        ...
        rule turns into ``pointer-events: none`` ‚Ä?no inline style. The
    ...
    def test_label_canvas_click_through(self, browser, tmp_path):
        ...
        ‚Ä?the shape ``AnnotationCanvas`` uses.  Without that it would swallow
    ...
    def test_clickable_data_canvas(self, browser, tmp_path):
        ...
        paints into ‚Ä?Leaflet hit-tests a canvas renderer by coordinate, not
        by element ‚Ä?so the CSS switches canvases back on.  A canvas that
        ...
        with use_page(self._make_page, browser, tmp_path, slug="clickable_data") as (
            ...
            assert result["canvasPointerEvents"] == "auto", (
                "a canvas in a pane must be re-enabled ‚Ä?the pane is "
            ...
            # even when nothing can reach it ‚Ä?the hole the pane-level
    ...
    def test_pane_svg_does_not_capture_the_map(self, browser, tmp_path):
        ...
        a layer painted below it ‚Ä?geometry hover/click dies and the cursor
        ...
        with use_page(self._make_page, browser, tmp_path, slug="pane_svg") as (
            ...
            # target.  This is the regression ‚Ä?the pane's SVG used to take
    ...
    def test_annotation_each_layer_own_pane_ordered_by_layer(self, browser, tmp_path):
        ...
        class alongside the annotation role marker ‚Ä?that is what makes the
        ...
        with use_page(self._make_page, browser, tmp_path) as (page, errors):
            ...
            assert result is not None and result["canvas"] is True, result
            # One pane + one canvas per labelled layer ‚Ä?not a shared canvas.
            ...
            # upper layer's labels cover the lower layer's ‚Ä?the stack order.
    ...
    def test_annotation_focus_lifts_label_pane(self, browser, tmp_path):
        ...
        with use_page(self._make_page, browser, tmp_path) as (page, errors):
            ...
            # (FOCUS.PANE_Z 9000 ‚à?FOCUSED_Z_GAP 10 + 1).
    ...
    def test_opacity_pane_versus_features(self, browser, tmp_path):
        ...
        After the ordering pass every overlay layer ‚Ä?a plain folium one
        included ‚Ä?owns a pane and has its content migrated into it, so the
        ...
        with use_page(self._make_page, browser, tmp_path) as (page, errors):
            ...
            # Case C: a hollow polygon (fillOpacity: 0) keeps its hole ‚Ä?the
            # pane's CSS opacity is multiplicative at compositing time, not an
            # override of the feature's own style.  0 √ó 0.4 = 0.
    ...
    def test_toggle_all_checkbox_toggles_layers(self, browser, tmp_path):
        ...
        with use_page(
            ...
            # Check initial state ‚Ä?all overlays checked
    ...
    # ‚îÄ‚îÄ title / tooltip browser tests ‚îÄ‚îÄ
    ...
    def test_rename_keeps_checkbox_title_and_sets_aria_label(self, browser, tmp_path):
        ...
        with use_page(
            ...
            # The name reaches assistive tech via aria-label‚Ä?
            assert res["ariaLabel"] == "RenamedLayer", (
                f"aria-label={res['ariaLabel']!r}"
            )
            # ‚Ä¶but never replaces the Select/Deselect tooltip (accept the
            # translated wording ‚Ä?a local browser can resolve `zh`).
            assert res["title"] in ("Deselect", "ÈöêËóè"), f"title={res['title']!r}"
    ...
    def _assert_toggle_all_deselect_title(self, title: str | None, label: str) -> None:
        ...
        translated wording too ‚Ä?the contract is that the tooltip is the
        toggle-all action, never a layer name.
        """
        assert title and ("Deselect" in title or "ÈöêËóè" in title), (
    ...
    def test_toggle_all_row_title_shows_fold_unfold(self, browser, tmp_path):
        ...
        with use_raw_page(browser.new_page) as page:
            ...
            # Expanded ‚Ü?should show "Collapse layers"
            ...
            # Folded ‚Ü?should show "Expand layers"
    ...
    def test_fold_preserves_dom_index(self, browser, tmp_path):
        ...
        with use_raw_page(browser.new_page) as page:
            ...
            # Count DOM items after fold ‚Ä?should still be same (not removed)
            after = page.evaluate(
                "document.querySelectorAll('.foliplus-layer-item:not(.foliplus-color-layer-item)').length"
            )
            assert after == before, (
                f"Expected {before} items after fold, got {after} ‚Ä?DOM items should not be removed"
            )
    ...
    def test_fold_svg_switches_on_toggle(self, browser, tmp_path):
        ...
        with use_raw_page(browser.new_page) as page:
            ...
            # Single SVG, 1 path before fold (SVGO converts polyline ‚Ü?path)
            ...
            # Still 1 path ‚Ä?icon is rotated by CSS, not swapped
            elem_count = page.evaluate(_js("LayerControl/count_fold_paths"))
            assert elem_count == 1, (
    ...
    # ‚îÄ‚îÄ Indeterminate checkbox browser tests ‚îÄ‚îÄ
    ...
    def test_toggle_all_indeterminate_state(self, browser, tmp_path):
        ...
        with use_raw_page(browser.new_page) as page:
            ...
            # Step 1: All checked ‚Ü?toggle-all should be checked (not indeterminate)
            ...
            # Step 2: Uncheck one layer ‚Ü?toggle-all should be indeterminate
            ...
            # Step 3: Uncheck all layers ‚Ü?toggle-all should be unchecked (not indeterminate)
    ...
    def test_toggle_all_click_indeterminate_deselects_all(self, browser, tmp_path):
        ...
        with use_raw_page(browser.new_page) as page:
            ...
            # Click toggle-all (indeterminate ‚Ü?deselect all)
            page.evaluate(_js("LayerControl/click_toggle_all"))
            page.wait_for_timeout(300)
    ...
    def test_paneset_reset_after_hide_show(self, browser, tmp_path):
        ...
        Uses a FeatureGroup with a child marker ‚Ä?the marker (leaf) is what
    ...
    def test_pane_zindex_hierarchy(self, browser, tmp_path):
        ...
        Regression: markers (search/locate pins, ‚ú? data markers) must render
        ...
        with use_page(
            ...
            # GeoJSON's layer is resolved after its addTo fires layeradd ‚Ü?
    ...
    def test_layers_view_is_readonly(self, browser, tmp_path):
        """api.layers is a read-only view ‚Ä?direct mutation is blocked.
    ...
    def test_row_cursor_renders_glow_on_clear_surface(self, browser, tmp_path):
        ...
        DOM ‚Ä?explicit white surface + red glow ‚Ä?for BOTH the keyboard cursor
        ...
        with use_page(self._make_page, browser, tmp_path, overlay1, overlay2) as (
            ...
            panel_ready(page)
            # Force a truly unchecked row ‚Ä?do not trust show=False across
    ...
    def test_row_cursor_turns_row_white_on_hover_tab_and_arrow(self, browser, tmp_path):
        ...
        The interaction target always paints `var(--neutral-0)` ‚Ä?including on
        ...
        with use_page(self._make_page, browser, tmp_path, overlay1, overlay2) as (
            ...
            # Checked: rest wash ‚Ü?cursor white ‚Ü?wash returns.
    ...
    def test_fold_row_cursor_wakes_glow_and_red_icon(self, browser, tmp_path):
        ...
        (white surface + glow) and wakes the fold icon red, exactly like hover ‚Ä?
    ...
    def test_keydown_after_label_click_targets_clicked_row(self, browser, tmp_path):
        ...
        with use_page(self._make_page, browser, tmp_path, overlay1, overlay2) as (
            ...
            # focus as :focus-visible, so focusin can light the row ‚Ä?allowed.
    ...
    def test_keydown_escape_restores_rest_state(self, browser, tmp_path):
        ...
        grip stays hidden at rest either way ‚Ä?only the cursor recipe (arrow
        keys, Tab) and a real hover show it ‚Ä?and the more button returns to
        ...
        with use_page(self._make_page, browser, tmp_path, overlay) as (page, _):
            ...
            # ‚îÄ‚îÄ checked rows: the restored row must equal the reference ‚îÄ‚îÄ
            escape_first_row()
            ...
            # ‚îÄ‚îÄ unchecked rows: same contract against the plain surface ‚îÄ‚îÄ
            page.evaluate(
            ...
            # the primary text color ‚Ä?so Esc must return it to different
    ...
    def test_native_gate_range_slider_arrowdown(self, browser, tmp_path):
        ...
        ``defaultPrevented``. Both halves of the gate are measured ‚Äî the native
    ...
    def test_native_gate_checkbox_space(self, browser, tmp_path):
        ...
        row-toggle shortcut must not run alongside the native flip ‚Äî otherwise
    ...
    def test_base_basemap_quiet_focus(self, browser, tmp_path):
        ...
        Basemaps are stack slots, not data layers ‚Ä?click / keyboard focus
        ...
        with use_page(self._make_page, browser, tmp_path, overlay, base) as (page, _):
            ...
            # Checked base row: no glow, not white ‚Ä?keep the visibility wash.
    ...
    def test_focus_overlay_pane_keeps_spotlight_visible(self, browser, tmp_path):
        ...
        the overlay pane carries ``foliplus-layer-pane`` ‚Ä?the semantic marker
        ...
        with use_page(
            ...
            # owned pane has climbed to or above it ‚Ä?otherwise the mask would
            ...
            assert result["withoutExclusion"] == "hidden", (
                "dropping the exclusion tag must hide the overlay pane ‚Ä?this is "
    ...
    def test_focus_overlay_pane_click_through(self, browser, tmp_path):
        ...
        from focus.css, so the pane div ‚Ä?which the SVG renderer fills across
        the whole map while a focus is live ‚Ä?never eats a click. Without the
    ...
    def test_focus_overlay_renderer_lifecycle(self, browser, tmp_path):
        ...
        renderer), so every focus cycle must leave the pane empty ‚Ä?the pane
        ...
        with use_page(
            ...
            for cycle in result["cycles"]:
                ...
                assert cycle["after"]["svg"] == 0, (
                    f"cancel must remove the SVG renderer ‚Ä?a non-zero count is a "
                    f"leak: {cycle}"
                )
    ...
    def test_panel_stays_open_on_outside_press_by_default(self, browser, tmp_path):
        ...
        and the document-level capture/bubble pair sees real propagation ‚Ä?not a
    ...
    def test_panel_collapses_on_outside_press_when_enabled(self, browser, tmp_path):
        ...
        Proves the parameter is wired end to end ‚Ä?Python constructor -> CONF ->
        shell -> ``bindOutsideCollapse`` ‚Ä?rather than accepted and ignored.
        """
    ...
    def test_row_overlay_stays_visible_over_lit_siblings(self, browser, tmp_path):
        """Sibling rows keep their hover wake while a row overlay is open ‚Ä?
        ...
        Probes both the ‚ã?menu and the attrs panel: hovering a sibling
        ...
        with use_page(self._make_page, browser, tmp_path, *layers) as (page, _):
            ...
            def row_wake(idx):
                ...
                # Nothing may suppress it ‚Ä?it is the row's own state display.
                return page.evaluate(
            ...
            def overlay_topmost(sel):
                ...
                # itself ‚Ä?a lit sibling would otherwise paint over it.
                return page.evaluate(
            ...
            # ‚îÄ‚îÄ ‚ã?menu open: a hovering sibling wakes, the menu stays topmost ‚îÄ‚îÄ
            ...
            # ‚îÄ‚îÄ attrs panel open: same contract ‚îÄ‚îÄ
            # Escape closes the menu without collapsing the panel (a click
            # outside the panel would ‚Ä?bindOutsideCollapse).
    ...
    # ‚îÄ‚îÄ R7 zoom-range browser probes (¬ß31) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
    #
    ...
    def test_zoom_range_native_tilelayer(self, browser, tmp_path):
        ...
        with use_page(self._make_page, browser, tmp_path, t1, slug="zr_nat") as (
            ...
            # Leaflet does not self-apply options.minZoom changes ‚Ä?the
            # adapter reset is what makes the range visible (¬ß6.2).
    ...
    def test_zoom_range_reload_does_not_add_author_hidden_layer(
        ...
        to re-add a layer the author left off the map ‚Ä?only ``applyUserState``'s
        ...
        with use_raw_page(browser.new_page) as page:
            ...
            # applyZoomRangeStateOne ‚Ä?which must NOT add the layer to the map.
            page.evaluate("""
                () => {
                    const el = document.querySelector('.leaflet-container');
                    const map = (el && window[el.id]) || window.map;
            ...
            # ADD a layer the author left off ‚Ä?if folium already placed it,
    ...
    # ‚îÄ‚îÄ Row lookup by data-layer-id, not by registry / DOM position ‚îÄ‚îÄ
    #
    ...
    def test_initlayeritem_updates_only_the_id_match_row(self, browser, tmp_path):
        ...
        with use_page(self._make_page, browser, tmp_path, slug="initlayeritem") as (
            ...
            assert state["labels"]["beta"] == "B", (
                ...
                f"({state['staleIndexWouldHaveHit']!r}) ‚Ä?row resolved by index"
            )
...
# ‚îÄ‚îÄ R1 pane-surface probe (¬ß10.3) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
#
...
class TestLayerPaneProbeBrowser:
    """R1 probe ‚Ä?measured pane-surface facts (¬ß10.3 ten-item checklist)."""

    @staticmethod
    def _probe(browser, tmp_path, *layers, slug="probe"):
        """Render a default folium map (OSM base ‚Ü?finite maxZoom, so
    ...
    # ‚îÄ‚îÄ R3: materialize at registration (I1 / I2) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ

    def test_register_materializes_pane_before_add(self, browser, tmp_path):
        """I1: after ``registerLayer`` returns ‚Ä?and *before* any ordering pass ‚Ä?
    ...
    # ‚îÄ‚îÄ overlay / plugin probes (¬ß10.3 #1‚Ä?6) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ

    def test_probe_geojson_mixed_geometry(self, browser, tmp_path):
        """#1 GeoJson: point‚Üímarker, line/polygon‚Üípath, all in one pane."""
    ...
    def test_probe_standalone_marker_unregistered(self, browser, tmp_path):
        ...
        ‚Ä?its divIcon stays in the shared markerPane; setOpacity is the
    ...
    def test_probe_third_party_featuregroup_unregistered(self, browser, tmp_path):
        ...
        not registered) splits across overlayPane/markerPane ‚Ä?both shared."""
    ...
    def test_probe_third_party_featuregroup_registered(self, browser, tmp_path):
        ...
        the C-tier contract ‚Ä?register to get the pane carrier."""
    ...
    # ‚îÄ‚îÄ base + mixed renderer probes (¬ß10.3 #7‚Ä?10) ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
    ...
    def test_probe_two_tilelayers_native_opacity(self, browser, tmp_path):
        ...
        leaves the sibling opaque ‚Ä?direct evidence for "Â∫ïÂõæËµ∞ÂéüÁî?setOpacity,
        Ê∞∏‰∏çÁî?pane ËΩΩ‰Ωì"."""
    ...
    def test_probe_color_basemap_no_pane(self, browser, tmp_path):
        """#9 Solid-color basemap has NO pane/element today ‚Ä?it is the map
        ...
        # No foliplus-owned pane carries the color ‚Ä?only the tile-hidden
    ...
    def test_probe_mixed_renderer_pane_consistent(self, browser, tmp_path):
        ...
        assert r["hitHiddenIsCanvas"] is False  # hidden label pane ‚Ü?path hit
        assert r["hitNoPointerIsCanvas"] is False  # pointer-events:none ‚Ü?path hit
