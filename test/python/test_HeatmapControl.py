...
class TestHeatmapControlRendering:
    ...
    def test_locale_zh(self):
        ...
        assert_locale(html, "ÁΩëÊ†ºËÅöÂêà")

    def test_field_kwarg_removed(self):
        """field kwarg is no longer accepted ‚Ä?it was a dead declaration."""
    ...
    # ‚îÄ‚îÄ Performance optimization tests ‚îÄ‚îÄ
    ...
    # ‚îÄ‚îÄ Algorithm tests (rendering checks) ‚îÄ‚îÄ
...
class TestHeatmapControlBrowser:
    ...
    def _make_page(
        ...
        **ctrl_kwargs,
        ...
        prelude
            Optional JS source installed before the page's own scripts run ‚Ä?
    ...
    def test_remove_readd_leaves_no_listener_residue(self, browser, tmp_path):
        ...
        rounds[2] follow remove‚Üíadd. A listener leak ‚Ä?whether it lands on
        round[0] or only shows up in a later round ‚Ä?registers as a drift
    ...
    def test_remove_readd_leaves_no_document_listener_residue(self, browser, tmp_path):
        """Two remove‚Üíadd cycles with the dropdown open must not accumulate
        ...
        removing the control, so a listener still bound at removal time ‚Ä?a bare
        listener with no cleanup, or one bound to a controller that is never
        aborted ‚Ä?registers as drift.
        ...
        with use_page(
            ...
            assert all(o > c for o, c in zip(state["open"], state["closed"])), (
                ...
                f"listener (open={state['open']!r}, closed={state['closed']!r}) ‚Ä?"
    ...
    def test_probe_leak_listener_doc_control_group_moves(self, browser, tmp_path):
        ...
        hook in the shared probe ‚Ä?the part that keeps a signal-managed listener
        from registering as a phantom leak ‚Ä?would go untested.
        """
        with use_page(
            self._make_page,
            browser,
    ...
    def test_panel_collapses_on_outside_press_by_default(self, browser, tmp_path):
        ...
        its historic behaviour for any caller that does not opt out ‚Ä?and only
    ...
    def test_ui_control_changes_persist(self, browser, tmp_path):
        ...
        with use_page(
            ...
            # label toggle ‚Ä?rendered by the shared label-controls module, so
    ...
    def test_uncommitted_borderweight_survives_reload(self, browser, tmp_path):
        ...
        and re-rendered but never called ``saveConfig`` ‚Ä?only the ``onchange``
        ...
        with use_page(
            ...
            # Focus + set value + fire INPUT only ‚Ä?no change event, no blur.
            page.evaluate(
                """() => {
    ...
    def test_cached_agg_invalidation_on_layer_change(self, browser, tmp_path):
        ...
        with use_page(self._make_page, browser, tmp_path, expose_ctrl=True) as (
            ...
            # (replaces the old raw map.fire('layeradd') ‚Ä?the manager now
...
class TestHeatmapAutoFieldBrowser:
    ...
    def test_auto_field_priority_order(self, browser, tmp_path):
        ...
        ``_value`` ‚Ü?``options.value`` ‚Ü?``feature.properties`` keys.  Only
        ...
        with use_page(self._build_page, tmp_path, browser, features, agg="sum") as (
            ...
            # enumeration order ‚Ä?the important thing is deterministic choice.
    ...
    def test_auto_field_single_field(self, browser, tmp_path):
        ...
        with use_page(self._build_page, tmp_path, browser, features, agg="avg") as (
            ...
            # Single field ‚Ü?pickAutoField returns it directly
            auto_key = page.evaluate("window.__heatmapCtrl.manager.autoFieldKey")
