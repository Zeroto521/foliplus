...
function collectExports(fp, seen = new Set(), depth = 0) {
  ...
  while ((m = STAR_RE.exec(s)))
    collectExports(resolve(dirname(p), m[1]), seen, depth + 1).forEach(x => n.add(x));
...
const shimPlugin = {
  ...
  load(id) {
    ...
    const code = [
      "var " + sn + " = globalThis." + ns(spec) + ";",
      ...names.map(n => "export const " + n + " = " + sn + "." + n + ";"),
    ].join("\n");
...
export default (name, entry) => ({
  ...
  onwarn(w) {
    if (
      !["UNRESOLVED_IMPORT", "MISSING_GLOBAL_NAME", "CIRCULAR_DEPENDENCY"].includes(
        w.code,
      )
    )
