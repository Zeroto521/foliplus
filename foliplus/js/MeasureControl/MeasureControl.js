...
const CONST = {
  ...
  STORAGE: {
    KEY: `foliplus_measure_${map.getContainer().id}`,
  },
  MODE: {
    MARKER: "marker",
    DISTANCE: "distance",
  ...
  async handleMarkerClick(e) {
    ...
    marker.on("popupopen", () => {
      ...
      if (measurement.address !== null)
        marker.setPopupContent(MeasureUtils.buildPopup(lng, lat, measurement.address));
...
class CircleMode extends PreviewMode {
  ...
  start() {
    ...
    const finalizeCircle = (centerLatLng, r, targetLatLng) => {
      ...
      const radiusNode = this.layers.addLayer(MeasureUtils.makeNode(finalTargetLatLng));
