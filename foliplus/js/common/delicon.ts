 ...
 */
const bindDelIconToPopup = (marker: L.Marker | null, _delIcon: L.Layer) => {
  if (!marker) return;
  marker.on("popupopen", () => toggleDelIcon(marker, true));
  marker.on("popupclose", () => toggleDelIcon(marker, false));
};
