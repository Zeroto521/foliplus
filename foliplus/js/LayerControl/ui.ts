...
class LayerUI {
  ...
  bindEvents() {
    ...
    // Double-click on a layer row → focus the map on that layer. Native widgets
    // (checkbox, color input, button) already have a double-click meaning, so
    // handleDblClick ignores those targets.
  ...
  unbindEvents() {
    ...
    // Flush, never cancel: the last checkbox change may still be sitting in the
    // debounce queue (100ms), and cancelling here would drop it on unload.
  ...
  handleKeyDown(event: KeyboardEvent): void {
    ...
    switch (event.key) {
      ...
      case "Enter":
        ...
        // Enter only toggles visibility when the row's own checkbox has the
        // keyboard focus (a row click never focuses it, so the row div itself
        // is not a shortcut either). Space is handled natively by the input.
  ...
  handleDblClick(event: MouseEvent): void {
    ...
    // Double-click is reserved for native widgets: the browser has one for
    // checkboxes, color inputs and buttons, and the event still bubbles to
    // the container. Focusing a layer while the checkbox toggles twice would
    // make one gesture produce three effects, so only the row's dead space
    // (label / count / icon area) counts as a row double-click.
  ...
  /** Enter on a focused layer row → toggle its visibility checkbox, no menu. */
