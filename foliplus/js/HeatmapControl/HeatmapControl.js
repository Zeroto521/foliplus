...
const CONST = {
  ...
  ID: "foliplus_heatmap",
  FIELD: CONF.field,
  SCHEME: CONF.color_scheme,
  METHOD: CONF.method,
  N_CLASSES: CONF.n_classes,
  BORDER: {
    W: CONF.border_weight,
    COLOR: CONF.border_color,
    FILL_OP: CONF.fill_opacity,
    OP: CONF.border_opacity,
  },
  LABEL: {
    SHOW: CONF.label_show,
    SIZE: CONF.label_size,
    COLOR: CONF.label_color,
    FORMAT: CONF.label_format,
  },
  AGG: {
    DEFAULT: CONF.agg,
    COUNT: "count",
...
class HeatmapControl extends L.Control {
  ...
  buildStyleSection() {
    ...
    foliplus.dom.el("button", {
      ...
      onclick: () => {
        ...
        this.syncSelect(this.classSelect, String(CONF.n_classes));
        this.syncSelect(this.methodSelect, CONF.method);
        this.schemeSelectHidden.value = CONF.color_scheme;
        this.labelChk.checked = CONF.label_show;
        this.borderWeightInput.value = CONF.border_weight;
        this.borderColorInput.value = CONF.border_color;
        this.updateSchemeBar();
        this.updateFieldSelector();
  ...
  buildLayerListItems(sel) {
    ...
    sel.onchange = () => {
      ...
      if (this.extraBody)
        this.extraBody.classList.toggle(CONST.CLASSES.HIDDEN, !this.m.selectedLayerId);
  ...
  refreshSchemeDropdownItems() {
    ...
    const items = this.schemeDropdown.querySelectorAll(CONST.SEL.SCHEME_DROPDOWN_ITEM);
    items.forEach((item) => {
  ...
  resetAll() {
    ...
    this.m.currentField = CONF.field;
    this.m.numClasses = CONF.n_classes;
    this.m.currentMethod = CONF.method;
    this.m.currentScheme = CONF.color_scheme;
    this.m.currentLabelShow = CONF.label_show;
    this.m.borderWeight = CONF.border_weight;
    this.m.borderColor = CONF.border_color;
    this.m.clearHeatmapCanvas();
  }
