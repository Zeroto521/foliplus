...
test: build-js-dev test-js
	npm run build:verify
	pytest -v -r a --color=yes -n $(JOBS) --cov=foliplus --cov=changelog_check --cov-append --cov-report=term-missing --cov-report=xml --junitxml=junit.xml -o junit_family=legacy test/python script

test-python: build-js-dev
	npm run build:verify
	pytest -v -r a --color=yes -n $(JOBS) -m "not browser" --cov=foliplus --cov=changelog_check --cov-append --cov-report=term-missing --cov-report=xml --junitxml=junit.xml -o junit_family=legacy test/python script

test-browser: build-js-dev
	npm run build:verify
	pytest -v -r a --color=yes -n $(JOBS) -m "browser" --cov=foliplus --cov-append --cov-report=term-missing --cov-report=xml --junitxml=junit-browser.xml -o junit_family=legacy test/python script
